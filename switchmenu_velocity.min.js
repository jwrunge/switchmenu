...
var settings={scrollTarget:'both',scrollDuration:500,outAnimation:'transition.slideLeftBigOut',inAnimation:'transition.slideRightBigIn',outSpeed:500,inSpeed:500};if(options!=null)
$.extend(settings,options);$(switch_out).velocity(settings.outAnimation,{duration:settings.outSpeed,complete:function(){if(settings.scrollTarget=='both'||settings.scrollTarget=='screen')
$('html, body').velocity('scroll',{duration:settings.scrollDuration,container:$(switch_in)});if(settings.scrollTarget=='both'||settings.scrollTarget=='body')
{$('html, body').velocity('scroll',{duration:settings.scrollDuration,complete:function(){$(switch_in).velocity(settings.inAnimation,{duration:settings.inSpeed,complete:function(){currently_switching=!1}})}})}
else $(switch_in).velocity(settings.inAnimation,{duration:settings.inSpeed,complete:function(){currently_switching=!1}})}})}
