...
{currently_switching=!0;var settings={scrollTarget:'both',scrollDuration:500,outAnimation:'transition.slideLeftBigOut',inAnimation:'transition.slideRightBigIn',outSpeed:500,inSpeed:500};if(options!=null)
$.extend(settings,options);$(switch_out).velocity(settings.outAnimation,{duration:settings.outSpeed,complete:function(){if(settings.scrollTarget=='both'||settings.scrollTarget=='body')
$('html, body').velocity('scroll',settings.scrollDuration);if(settings.scrollTarget=='both'||settings.scrollTarget=='screen')
$(switch_in).velocity('scroll',settings.scrollDuration);$(switch_in).velocity(settings.inAnimation,{duration:settings.inSpeed,complete:function(){currently_switching=!1}})}})}
...
{var screen_class='.'+$(this.menu_selector).prop('id')+'_screen';switchin('#'+$(screen_class).first().prop('id'),screen_class,!1);this.menu.children('a').removeClass('selected');this.menu.children("a[href='"+'#'+$(screen_class).first().prop('id')+"']").addClass('selected')}
this.init=function()
